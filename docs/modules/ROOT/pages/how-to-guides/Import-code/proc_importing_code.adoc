...
. Click *Docker Configuration*. 
. To display the Dockerfile registry authentication code, download the authentication of your new account or click *view its contents.* 
. Enter the Dockerfile registry authentication code. It should look similar to this example:
```
   {
    "auths": {
        "registry.redhat.io": {
...
. In the {ProductName} UI, go to your application components and click on *Edit component settings*.  
...
.. If you have not previously created a secret with this name, then it should be successfully created. If you have already created a secret with the *registry-redhat-io* name, you must delete the current secret before being able to recreate it.
.. If you need to delete the secret, you can delete it from your command line by running the `oc secrets link appstudio-pipeline registry-redhat-io` command. 
