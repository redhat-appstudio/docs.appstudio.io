...
. To display the Dockerfile registry authentication, download the authentication of your new account or click *view its contents.*
. Enter the Dockerfile registry authentication. It should look similar to this example:
```
   {
    "auths": {
        "registry.redhat.io": {
...
... You can delete it from your command line by running the `oc secrets link appstudio-pipeline registry-redhat-io` command.
