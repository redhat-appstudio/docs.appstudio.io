...
For Java application Components in {ProductName}, you can configure the Java Virtual Machine (JVM) build service either with the dependencies rebuild feature enabled or with that feature disabled.
...
The Java ecosystem uses a binary distribution model, where binary jar files are downloaded from central repositories, such as Maven Central. This distribution model means that the only way to ensure a Component is completely built from source is to rebuild all its component libraries from source in a trusted environment. Due to the Java ecosystem, you would need to manually rebuild the component libraries.
...
* The system builds your Component by using community dependencies.

* The JVM Build Service analyzes your Component and determines which dependencies the system needs to rebuild.

* The JVM Build Service generates `ArtifactBuild` objects to represent each artifact in your Component before determining to which repository the artifacts belong. The JVM Build Service tags from which systems the artifacts derive.
...
* When the system has completed all its builds, you can rebuild your Component.

* If all the builds are successful, your Component's SBOM displays that none of the dependencies came from third-party repositories. However, you may need to troubleshoot when some dependencies fail to build.
...
. In the RHTAP UI create an image pull secret for the quay.io repository using the page under the 'Secrets' tab. This can be called anything you like.
. Create a file, for example, `config.yaml`.
. In the `config.yaml` file, create a *JBSConfig* resource with the following data:
...
spec:
  ...
  mavenBaseLocations:
    maven-repository-300-jboss: "https://repository.jboss.org/nexus/content/groups/public/" <6>
...
When building a Component, the system redirects all requests by using this artifact cache. By default this proxies to maven central, however, you can configure other repositories. The repositories configured in your project will not be used,
